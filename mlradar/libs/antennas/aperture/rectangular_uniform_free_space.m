classdef rectangular_uniform_free_space
    % Rectangular aperture, uniform distribution free space
    % Created by: Lee A. Harrison
    % On: 8/2/2018
    
    properties
        % Properties for rectangular aperture, uniform free space
    ...
    methods
        % Methods for rectangular aperture, uniform free space
        function obj = rectangular_uniform_free_space(width, height, frequency, r, theta, phi)
            % Class constructor
            if(nargin > 0)
                obj.r = r;
                obj.theta = theta;
        ...
        function [half_power_eplane, half_power_hplane, first_null_eplane, first_null_hplane] = beamwidth(obj)
            ...
            half_power_hplane = 50.6 * wavelength / obj.width;
            ...
            first_null_hplane = 114.6 * wavelength / obj.width;
        ...
        function d = directivity(obj)
            ...
            d = 4.0 * pi * obj.width * obj.height / wavelength ^ 2;
        end
        ...
        function [e_r, e_theta, e_phi, h_r, h_theta, h_phi] = far_fields(obj)
            ...
            e_theta = 1j * obj.width * obj.height * k / (pi * obj.r) * exp(-1j * k * obj.r) * sin(obj.phi) .* (1.0 + cos(obj.theta)) .* ...
                sinc(k * obj.width * 0.5 * sin(obj.theta) .* cos(obj.phi)) .* sinc(k * obj.height * 0.5 * sin(obj.theta) .* sin(obj.phi));
            
            % Define the phi-component of the electric far field (V/m)
            e_phi = 1j * obj.width * obj.height * k / (pi * obj.r) * exp(-1j * k * obj.r) * cos(obj.phi) .* (1.0 + cos(obj.theta)).* ...
                sinc(k * obj.width * 0.5 * sin(obj.theta) .* cos(obj.phi)) .* sinc(k * obj.height * 0.5 * sin(obj.theta) .* sin(obj.phi));
            ...
            h_theta = -1j * obj.width * obj.height * k / (pi * eta * obj.r) * exp(-1j * k * obj.r) * cos(obj.phi) .* (1.0 + cos(obj.theta)) .* ...
                sinc(k * obj.width * 0.5 * sin(obj.theta) .* cos(obj.phi)) .* sinc(k * obj.height * 0.5 * sin(obj.theta) .* sin(obj.phi));
            
            % Define the phi-component of the magnetic far field (A/m)
            h_phi = 1j * obj.width * obj.height * k / (pi * eta * obj.r) * exp(-1j * k * obj.r) * sin(obj.phi) .* (1.0 + cos(obj.theta)) .* ...
                sinc(k * obj.width * 0.5 * sin(obj.theta) .* cos(obj.phi)) .* sinc(k * obj.height * 0.5 * sin(obj.theta) .* sin(obj.phi));
        end        

function attenuation = cloud_fog_attenuation(frequency, liquid_water_temperature, liquid_water_density)
...
eps_pp = frequency * (eps_0 - eps_1) ./ (fp * (1. + (frequency/fp).^2)) + frequency * (eps_1 - eps_2) ./ (fs * (1. + (frequency/fs).^2));
...
attenuation = k_l .* liquid_water_density;

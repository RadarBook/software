"""
Project: RadarBook
File: stretch_processor_example.py
...
class StretchProcessor(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.range_window_length.returnPressed.connect(self._update_canvas)
    ...
    def _update_canvas(self):
        ...
        self.axes1.plot(range_window, 20.0 * log10(abs(so) / number_of_samples + finfo(float).eps), '')
        self.axes1.set_xlim(min(t_range) - 5, max(t_range) + 5)
        self.axes1.set_ylim(-60, max(20.0 * log10(abs(so) / number_of_samples)) + 10)

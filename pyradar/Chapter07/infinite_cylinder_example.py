"""
Project: RadarBook
File: infinite_cylinder_example.py
...
class InfCyl(QMainWindow, Ui_MainWindow):
    ...
    def _update_canvas(self):
        ...
        # Display the results
        self.axes1.plot(observation_angle, 10.0 * log10(rcs_te), '', label='TE$^z$')
        self.axes1.plot(observation_angle, 10.0 * log10(rcs_tm), '--', label='TM$^z$')
        ...
        self.axes1.set_ylim(min(10.0 * log10(rcs_te)) - 3, max(10.0 * log10(rcs_te)) + 3)
...
def start(parent):
    form = InfCyl(parent)  # Set the form
    form.show()      # Show the form


def main():
    app = QApplication(sys.argv)  # A new instance of QApplication
    form = InfCyl()               # Set the form
    form.show()                   # Show the form

"""
Project: RadarBook
File: burnthrough_range_example.py
...
"""
import sys
from Chapter11.ui.Burnthrough_ui import Ui_MainWindow
...
class Burnthrough(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.jammer_to_signal_req.returnPressed.connect(self._update_canvas)
    ...
    def _update_canvas(self):
        ...
        if jammer_type == 'Self Screening':
            kwargs = {'peak_power': float(self.radar_transmit_power.text()),
                      ...
                      'jammer_to_signal_req': 10 ** (float(self.jammer_to_signal_req.text()) / 10.0)}

            # Calculate the jammer to signal ratio
            burnthrough_range = countermeasures.burn_through_range_selfscreen(**kwargs)
        ...
        self.axes1.set_title('Burn Through Range', size=14)
        self.axes1.set_xlabel('Jammer ERP (dBW)', size=12)
        self.axes1.set_ylabel('Burn Through Range (m)', size=12)
...
def start(parent):
    form = Burnthrough(parent)  # Set the form
    form.show()           # Show the form


def main():
    app = QApplication(sys.argv)  # A new instance of QApplication
    form = Burnthrough()          # Set the form
    form.show()                   # Show the form

"""
Project: RadarBook
File: rectangular_uniform_ground_plane.py
...
from numpy import sin, cos, sinc, exp, sqrt
...
def far_fields(width, height, frequency, r, theta, phi):
    ...
    e_theta = 1j * width * height * k / (2.0 * pi * r) * exp(-1j * k * r) * sin(phi) * \
    ...
    e_phi = 1j * width * height * k / (2.0 * pi * r) * exp(-1j * k * r) * cos(theta) * cos(phi) * \
            sinc(k * width * 0.5 * sin(theta) * cos(phi)) * sinc(k * height * 0.5 * sin(theta) * sin(phi))
    ...
    h_theta = -1j * width * height * k / (2.0 * pi * eta * r) * exp(-1j * k * r) * cos(theta) * cos(phi) * \
              sinc(k * width * 0.5 * sin(theta) * cos(phi)) * sinc(k * height * 0.5 * sin(theta) * sin(phi))

    # Define the phi-component of the magnetic far field (A/m)
    h_phi = 1j * width * height * k / (2.0 * pi * eta * r) * exp(-1j * k * r) * sin(phi) * \
            sinc(k * width * 0.5 * sin(theta) * cos(phi)) * sinc(k * height * 0.5 * sin(theta) * sin(phi))
